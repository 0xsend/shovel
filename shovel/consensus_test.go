...
func TestConsensus_PreventsMissingLogs(t *testing.T) {
	...
	// Use 2 different real Ethereum RPCs to demonstrate multi-provider consensus
	// This tests that different RPC providers return consistent data for block 17943843
	quickNodeURL := "https://practical-flashy-research.quiknode.pro/caae41d41b732f1f0872c8fa3ca2fe66ebb76a93/"
	alchemyURL := "https://eth-mainnet.g.alchemy.com/v2/demo" // Public demo endpoint
	
	// Quick connectivity check for QuickNode - skip test if not accessible
	testReq := map[string]any{"jsonrpc": "2.0", "method": "eth_blockNumber", "params": []any{}, "id": 1}
	testBody, _ := json.Marshal(testReq)
	testResp, err := http.Post(quickNodeURL, "application/json", bytes.NewReader(testBody))
	if err != nil || testResp.StatusCode != 200 {
		t.Skipf("Skipping test: QuickNode RPC not accessible (error: %v)", err)
		if testResp != nil {
			testResp.Body.Close()
		}
		return
	...
	faultyServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// For all other methods (eth_getBlockByNumber, eth_getBlockByHash, etc.), proxy to QuickNode
		resp, err := http.Post(quickNodeURL, "application/json", bytes.NewReader(body))
		if err != nil {
	...
	// - Provider A: Faulty (returns empty eth_getLogs, proxies other methods to QuickNode)
	// - Provider B: QuickNode (returns 4 events - same as integration tests)
	// - Provider C: Alchemy (returns 4 events - different provider for consensus verification)
	providerA := jrpc2.New(faultyServer.URL)
	providerB := jrpc2.New(quickNodeURL)
	providerC := jrpc2.New(alchemyURL)
