...
func TestReceiptValidation_DetectsConsensusOnWrongData(t *testing.T) {
	...
	// Phase 2: Receipt validator uses DIFFERENT provider (QuickNode with correct data)
	receiptProvider := jrpc2.New(quickNodeURL)
	rv := NewReceiptValidator(receiptProvider, true, NewMetrics(srcName))
...
func TestReceiptValidation_PassesWithCorrectData(t *testing.T) {
	...
	quickNodeURL := "https://practical-flashy-research.quiknode.pro/caae41d41b732f1f0872c8fa3ca2fe66ebb76a93/"
	alchemyURL := "https://eth-mainnet.g.alchemy.com/v2/demo"
	...
	testResp, err := http.Post(quickNodeURL, "application/json", bytes.NewReader(testBody))
	if err != nil || testResp.StatusCode != 200 {
		t.Skipf("Skipping test: RPC not accessible (error: %v)", err)
		if testResp != nil {
			testResp.Body.Close()
		}
		return
	...
	// Phase 1: Consensus with 2 correct providers
	consensusProviders := []*jrpc2.Client{
		jrpc2.New(quickNodeURL),
		jrpc2.New(alchemyURL),
	}
	...
	// Phase 2: Receipt validator uses QuickNode (different from Alchemy in consensus)
	receiptProvider := jrpc2.New(quickNodeURL)
	rv := NewReceiptValidator(receiptProvider, true, NewMetrics(srcName))
