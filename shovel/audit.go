...
func (a *Auditor) check(ctx context.Context) error {
	...
	for _, sc := range a.conf.Sources {
		...
		// Track queue length for this source
...
func (a *Auditor) verify(ctx context.Context, sc config.Source, t auditTask) error {
	...
	if matches < k {
		...
		wgAll.Wait()
		if allMatches == len(providers) && len(providers) > 0 {
			matches = k
		}
	}
	...
	`
	var retryCount int
	if err := a.pgp.QueryRow(ctx, checkRetryQ, t.srcName, t.igName, t.blockNum).Scan(&retryCount); err != nil {
		switch {
	...
	if retryCount >= maxAuditRetries {
		const fail = `
			...
			WHERE src_name = $1 AND ig_name = $2 AND block_num = $3
		`
		if _, err := a.pgp.Exec(ctx, fail, t.srcName, t.igName, t.blockNum); err != nil {
	...
	// task loop. We run the delete inside its own transaction so that
	// pg_advisory_xact_lock() is scoped to this unit of work.
	tx, err := a.pgp.Begin(ctx)
	if err != nil {
		return fmt.Errorf("starting delete tx: %w", err)
	}
	defer tx.Rollback(ctx)

	if _, err := tx.Exec(ctx, "select pg_advisory_xact_lock($1)", task.lockid); err != nil {
		return fmt.Errorf("acquiring task lock for delete: %w", err)
	}
